...
export default function Profile() {    
    ...
    const fetchUserData = (id: any, token: any) => {
        ...
        .then(res => {
            ...
            if (res.data.user.qrcodeurl === null) {
                ...
                // setQrcodeurl(res.data.user.qrcodeurl);
                jQuery("#googleAuth").attr('src', res.data.user.qrcodeurl);

                // const url = URL.createObjectURL(res.data.user.qrcodeurl);
                // setQrcodeurl(res.data.user.qrcodeurl);
                // jQuery("#googleAuth").attr('src', url);
            }
    ...
    const submitProfile = (event: any) => {
        ...
        mfaapi.put(`api/updateprofile/${userid}`, jsondata, { headers: {
            Authorization: `Bearer ${token}`
        }})
        .then((res: any) => {
    ...
    const enableMFA = () => {
        ...
        mfaapi.put(`api/mfa/activate/${userid}`, data, {headers: {
            Authorization: `Bearer ${token}`
        }})
        .then((res: any) => {
            setProfileMsg(res.data.message);
    ...
    const disableMFA = () => {
        ...
        mfaapi.put(`api/mfa/activate/${userid}`, jsonData, {headers: {
            Authorization: `Bearer ${token}`
        }})
        .then((res: any) => {
            if (res.data.qrcodeurl === null) {
    ...
    const changePassword = (event: any) => {
        ...
        mfaapi.put(`api/changepassword/${userid}`, jsonData, {headers: {
            Authorization: `Bearer ${token}`
        }})
        .then((res: any) => {
                setProfileMsg(res.data.message);
    ...
    return (
      <div className='profile-bg'>
        ...
        <form encType="multipart/form-data" autoComplete='false'>
                <div className='row'>
                    <div className='col'>
                        <input className="form-control bg-warning border-primary" id="firstname" name="firstname" type="text" value={fname} onChange={e => setFname(e.target.value)} required  />
                        <input className="form-control bg-warning border-primary mt-2" id="lastname" name="lastname" type="text" value={lname} onChange={e => setLname(e.target.value )} required />
                    </div>
                ...
                <div className='row'>
                    ...
                    <div className='col'>
                        ...
                        { showpwd === true ? (
                            <>
                            <input className="form-control text-white border-primary mt-2" type="password" id="newPassword" value={newpassword} onChange={e => setNewPassword(e.target.value)} placeholder='enter new Password'/>
                            <input className="form-control text-white border-primary mt-1" type="password" id="confNewPassword" value={confnewpassword} onChange={e => setConfNewPassword(e.target.value)} placeholder='confirm new Password'/>
